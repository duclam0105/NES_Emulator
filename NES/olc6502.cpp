...
olc6502::olc6502() {
	using a = olc6502;
	// 6052 16x16 table instruction - Find this in datasheet - Comment by LamND
	lookupTable =	//{ <name>, <func ptr to opcode>, <func ptr to addrMode>, <machine cycles to execute instruction>}
	{
...
uint8_t olc6502::read(uint16_t addr) {
	return bus->readBus(addr, false);
}

void olc6502::write(uint16_t addr, uint8_t data) {
	bus->writeBus(addr, data);
}
...
void olc6502::clock() {
	if (cycles == 0) {
		...
		uint8_t addtional_cycle1 = (this->*lookupTable[opcode].addrmode)();

		uint8_t addtional_cycle2 = (this->*lookupTable[opcode].operate)();

		cycles += (addtional_cycle1 & addtional_cycle2);
	}
...
uint8_t olc6502::ZP0() {
	addr_abs = read(PC);
	PC++;
	addr_abs &= 0x00FF;		//read low bytes
	return 0;
}
...
uint8_t olc6502::ABS() {
	...
	PC++;
	uint16_t higbytes = read(PC);
	PC++;
	addr_abs = (higbytes << 8) | lowbytes;
	return 0;
}
...
uint8_t olc6502::fetch() {
	if (!(lookupTable[opcode].addrmode == &olc6502::IMP)) {
...
uint8_t olc6502::ADC()
{
	...
	// The signed Overflow flag is s	et based on all that up there! :D
...
// of M, the data(!) therfore we can simply add, exactly the same way we did before.
...
map<uint16_t, std::string> olc6502::disassemble(uint16_t nStart, uint16_t nStop)
{
